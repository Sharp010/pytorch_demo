...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(2, 64, kernel_size=(3,3), stride=(2,2))
...
for epoch in range(150):
    ...
    for step, (img, label) in enumerate(dataloader):
        ...
        with torch.no_grad():
            d_epoch_loss_real += d_real_loss.item()
            d_epoch_loss_fake += d_fake_loss.item()
            g_epoch_loss += g_loss.item()

    with torch.no_grad():
        print("epoch {}| real_loss {:.4f} | fake_loss {:.4f}  | g_loss {:.4f}".format(epoch, d_epoch_loss_real,
                                                                                      d_epoch_loss_fake, g_epoch_loss))
torch.save(dis, "D.pth")
torch.save(gen, "G.pth")

# test
device='cuda' if torch.cuda.is_available() else 'cpu'
gen=torch.load("./G.pth",map_location=torch.device('cpu'))
random_noise_test=torch.randn(6,100).to(device)
img_gen=gen(one_hot([6,5,8,7,5,7]).to(device),random_noise_test)
imgs=img_gen.view(-1,28,28).cpu().detach().numpy()
for i in range(6):
